...
async def get_message_counts(guild: Guild) -> File:
    ...
    conn = await get_db_connection()
    data = await conn.fetchmany('SELECT COUNT("M"."Id"), "U"."Username" FROM "Message" "M" INNER JOIN "User" "U" ON "U"."Id" = "M"."UserId" WHERE "M"."GuildId" = $1 GROUP BY "U"."Username" ORDER BY COUNT("M"."Id") DESC LIMIT 10', (guild.id,))
...
async def log_message_edit(before: Message, after: Message) -> None:
    ...
    row = await conn.fetchmany('SELECT "Id" FROM "Message" WHERE "Id" = $1', (before.id,)) # Check if before message exists
...
async def log_message_deletion(message: Message) -> None:
    ...
    row = await conn.fetchrow('SELECT "Id" FROM "Message" WHERE "Id" = %s', (message.id,))

    if not row:
        await create_message_log(message)
    try: 
        await conn.execute("""UPDATE "Message" SET "DeleteDateUTC" = %s, "Deleted" = %s, "UpdateDateUTC" = %s WHERE "Id" = %s""", (get_date(), 1, get_date(), message.id))
    except Exception as e:
