...
class TestAPI(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_weather_api_points_400(self):
        with patch('requests.get') as mocker:
            with patch('logging.Logger.exception') as logger:
    ...
    async def test_apod_api_singular_200(self):
        with patch('requests.get') as mocker:
            ...
            result_url, result_date = result[:2]
            result_title, result_explanation = result[2:]
            ...
            self.assertEqual(
                result_explanation[0], nasa_json['explanation']
            )
    ...
    async def test_apod_range_200(self):
        with patch('requests.get') as mocker:
            ...
            results = await get_astronomy_picture(
                '2024-12-05', '2024-12-20'
            )
            result_urls, result_dates = results[:2]
            result_titles, result_explanations = results[2:]
